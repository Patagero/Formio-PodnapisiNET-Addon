...
app.get("/subtitles/:type/:imdb/:extra?.json", (req, res) => {
  ...
  if (!fs.existsSync(baseDir)) return res.json({ subtitles: [] });

  const subtitles = fs.readdirSync(baseDir)
    .filter(f => f.endsWith(".srt"))
    .map(f => ({
      id: `${imdb}-${path.basename(f, ".srt")}`,
      lang: path.basename(f, ".srt"),
      format: "srt",
...
/* ===== ROOT ===== */
app.get("/", (req, res) => res.send("Podnapisi.NET local cache addon running"));
